...
class RuntimeContractValidator {
  ...
  void validate(Map<String, dynamic> manifest) {
    ...
    if (schemaVersion == null || schemaVersion < 1) {
      throw const RuntimeContractException('schema_version must be a positive integer');
    }
    ...
    if (screens is List) {
      for (final entry in screens) {
        if (entry is! Map) {
          throw const RuntimeContractException('every screen must be an object');
        }
        final screen = entry.cast<String, dynamic>();
        final name = screen['name'];
        final stac = screen['stac'];
        if (name is! String || name.trim().isEmpty) {
          throw const RuntimeContractException('every screen needs a non-empty name');
        }
    ...
    if (resources is List) {
      for (final entry in resources) {
        if (entry is! Map) {
          throw const RuntimeContractException('every resource must be an object');
        }
        final resource = entry.cast<String, dynamic>();
        final id = resource['resource_id'] ?? resource['id'];
        if (id is! String || id.trim().isEmpty) {
          throw const RuntimeContractException('every resource needs resource_id');
        }
