...
class RuntimeManifest {
  ...
  int get version => int.tryParse('${raw['version'] ?? raw['manifest_version'] ?? 0}') ?? 0;
  String? get themeMode => raw['theme_mode']?.toString();

  List<Map<String, dynamic>> get screens => (raw['screens'] as List? ?? const [])
...
class _StacRuntimeState extends State<StacRuntime> {
  ...
  void initState() {
    ...
    _screenName = widget.manifest.homeScreen.isEmpty
        ? (widget.manifest.screens.isNotEmpty ? '${widget.manifest.screens.first['name']}' : '')
